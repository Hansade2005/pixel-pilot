...
class StorageManager {
  ...
  // ============================================================================
  // TEAM WORKSPACE SUPPORT (NEW)
  // ============================================================================

  // Cache for workspace types to avoid repeated API calls
  private workspaceTypeCache = new Map<string, 'personal' | 'team'>()

  /**
   * Detect if workspace is personal or team
   * This is the KEY method that makes everything else work transparently
   */
  ...
  // ============================================================================
  // TEAM WORKSPACE FILE OPERATIONS (NEW)
  // These methods handle file operations for team workspaces via API
  // ============================================================================

  /**
   * Create file in team workspace via API
   */
  private async createTeamWorkspaceFile(file: Omit<File, 'id' | 'createdAt' | 'updatedAt'>): Promise<File> {
    ...
    if (!response.ok) {
      throw new Error('Failed to create team workspace file')
    }
  ...
  /**
   * Get files from team workspace via API
   */
  private async getTeamWorkspaceFiles(workspaceId: string): Promise<File[]> {
    ...
    if (!response.ok) {
      console.error('Failed to fetch team workspace files')
      return []
    }
  ...
  /**
   * Get single file from team workspace via API
   */
  private async getTeamWorkspaceFile(workspaceId: string, path: string): Promise<File | null> {
    const files = await this.getTeamWorkspaceFiles(workspaceId)
    return files.find(f => f.path === path) || null
  }

  /**
   * Update file in team workspace via API
   */
  private async updateTeamWorkspaceFile(
    ...
    const file = await this.getTeamWorkspaceFile(workspaceId, path)
    if (!file) return null

    const response = await fetch(`/api/teams/workspaces/${workspaceId}/files/${file.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  /**
   * Delete file from team workspace via API
   */
  private async deleteTeamWorkspaceFile(workspaceId: string, path: string): Promise<boolean> {
    // First get the file to find its ID
    const file = await this.getTeamWorkspaceFile(workspaceId, path)
    if (!file) return false
  ...
  // ============================================================================
  // ENHANCED FILE METHODS (Override existing methods to support both types)
  // ============================================================================

  /**
   * Create file - works for both personal and team workspaces
   * This is called by chat-v2/route.ts and will transparently route to the right storage
   */
  async createFileTeamAware(file: Omit<File, 'id' | 'createdAt' | 'updatedAt'>): Promise<File> {
    const type = await this.getWorkspaceType(file.workspaceId)

    if (type === 'team') {
      console.log('[StorageManager] Creating file in TEAM workspace:', file.path)
      return await this.createTeamWorkspaceFile(file)
    } else {
      console.log('[StorageManager] Creating file in PERSONAL workspace:', file.path)
      await this.init()
   ...
   */
  async getFilesTeamAware(workspaceId: string): Promise<File[]> {
    const type = await this.getWorkspaceType(workspaceId)

    if (type === 'team') {
      console.log('[StorageManager] Fetching files from TEAM workspace')
      return await this.getTeamWorkspaceFiles(workspaceId)
    } else {
      console.log('[StorageManager] Fetching files from PERSONAL workspace')
      await this.init()
      return await this.storage!.getFiles(workspaceId)
    }
  }

  /**
   * Get single file - works for both types
   */
  async getFileTeamAware(workspaceId: string, path: string): Promise<File | null> {
    const type = await this.getWorkspaceType(workspaceId)

    if (type === 'team') {
      return await this.getTeamWorkspaceFile(workspaceId, path)
    } else {
      await this.init()
      return await this.storage!.getFile(workspaceId, path)
    }
  }

  /**
   * Update file - works for both types
   */
  async updateFileTeamAware(workspaceId: string, path: string, updates: Partial<File>): Promise<File | null> {
    const type = await this.getWorkspaceType(workspaceId)

    if (type === 'team') {
      console.log('[StorageManager] Updating file in TEAM workspace:', path)
      return await this.updateTeamWorkspaceFile(workspaceId, path, updates)
    } else {
      console.log('[StorageManager] Updating file in PERSONAL workspace:', path)
      await this.init()
      return await this.storage!.updateFile(workspaceId, path, updates)
    }
  }
   ...
   */
  async deleteFileTeamAware(workspaceId: string, path: string): Promise<boolean> {
    const type = await this.getWorkspaceType(workspaceId)

    if (type === 'team') {
      console.log('[StorageManager] Deleting file from TEAM workspace:', path)
      return await this.deleteTeamWorkspaceFile(workspaceId, path)
    } else {
      console.log('[StorageManager] Deleting file from PERSONAL workspace:', path)
      await this.init()
      return await this.storage!.deleteFile(workspaceId, path)
    }
  }
}
