 ...
 * Uses REST API for SELECT queries, Management API for DDL operations
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Determine if this is a SELECT query or DDL operation
    const isSelectQuery = /^\s*SELECT\b/i.test(sql.trim())
    const isDdlOperation = /^\s*(CREATE|ALTER|DROP|GRANT|REVOKE|COMMENT|TRUNCATE|INSERT|UPDATE|DELETE)\b/i.test(sql.trim())

    if (isSelectQuery && !isDdlOperation) {
      // Use REST API for SELECT queries
      console.log('[SUPABASE API] Using REST API for SELECT query')
      ...
      try {
        ...
        if (!response.ok) {
          console.error('[SUPABASE API] REST API failed:', result)
          return NextResponse.json(
            {
              success: false,
              error: `REST API error: ${result.message || 'Unknown error'}`,
        ...
        console.log('[SUPABASE API] REST API query completed successfully')
        return NextResponse.json({
          success: true,
        ...
        console.warn('[SUPABASE API] REST API failed, falling back to Management API:', restError.message)
        // Fall through to Management API
      }
    }

    // Use Management API for DDL operations or as fallback
    console.log('[SUPABASE API] Using Management API for DDL/fallback')
    ...
    try {
      ...
      console.log('[SUPABASE API] Management API execution completed successfully')
      ...
      console.error('[SUPABASE API] Management API execution failed:', error)
