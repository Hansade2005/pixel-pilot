...
export async function GET(
  ...
  try {
    ...
    // Check if subdomain exists in Redis using the utility method
    // This ensures we get a properly parsed plain object
    const subdomainData = await subdomainUtils.get(subdomain);
    
    // Ensure we have valid subdomain data
    if (!subdomainData) {
    ...
    for (const storagePath of storagePaths) {
      try {
        ...
        console.log(`[Subdomain API] Attempting to fetch: ${publicUrl}`)
        ...
        console.error(`[Subdomain API] Error fetching ${storagePath}:`, fetchError)
      }
    }

    // Fallback HTML if no file found
    console.warn(`[Subdomain API] No files found for subdomain: ${subdomain}`)
